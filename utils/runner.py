...
def run_tool(
    ...
    try:
        ...
        elapsed = time.time() - start
        timeout_hint = int(timeout) if timeout is not None else int(elapsed)
        return ToolResult(
            tool_name=tool_name,
            lines=[],
            elapsed=elapsed,
            status="timeout",
            message=(
                f"timeout after {timeout_hint}s. "
                f"Use -nt for no timeout and full results on large targets"
            ),
        )
